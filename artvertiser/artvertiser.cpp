 ...
 * 
 * This code builds upon the 'multigl' sample from Bazar. It has been modified
 * to support texture and video-texture mapping to an OpenGL plane over the ROI.
 * The ROI in the model image is now read in from a file generated by the
 * training process. Fades, fonts, input hooks and other bits have pieces have
 * been added also.
 *
 * Archival video can be used instead of a capture interface also. This required
 * modification of the 'multigrab.cpp'. I also fixed a bug in
 * planar_object_recognizer::build_with_cache where corner values for the ROI
 * were only being set immediately after training, not on plain init.
 * 
 * Usage:
 * 
...
static bool init( int argc, char** argv )
{
	...
	// copy char model_file before destroyed by strcat
...
static void draw(void)
{
	...
	*/
	...
	if (frame_ok and augment == 1) {
		...
		/* this is prototype code for an occlusion mask built by comparison 
		 * of the tracked plane with that of the model image
