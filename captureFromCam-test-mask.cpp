...
int main(int argc, char **argv)
{
	...
	CvMat *mat = cvCreateMat(1, 8, CV_8UC1);
	CvMemStorage *storage = cvCreateMemStorage(0);
	CvSeq *ptseq = cvCreateSeq(CV_SEQ_ELTYPE_POINT, 0, sizeof(CvPoint), storage); 
	
	CvPoint pt0 = cvPoint(10, 10);
	CvPoint pt0 = cvPoint(10, 10);
	CvPoint pt0 = cvPoint(10, 10);
	CvPoint pt0 = cvPoint(10, 10);
	CvPoint pt0 = cvPoint(10, 10);
	cvSeqPush(ptseq, &pt0);
	cvCvtSeqToArray(ptseq, &storage, CV_WHOLE_SEQ );
	

	cvNamedWindow("webcam capture", CV_WINDOW_AUTOSIZE);
	cvNamedWindow("region out", CV_WINDOW_AUTOSIZE);
	for (;;)
	{
